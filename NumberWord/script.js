...
let word = numberToWord(203932123.22)
console.log(word)

function numberToWord(num) {
    if (parseInt(num) > -1) {
        let numberWord = lessThanQuadrillion(parseInt(num));
        if (num % 1 != 0) {
            let part = ((num * 100) - (Math.floor(num) * 100))
            let partWord = lessThanQuadrillion(part)
            numberWord += ' ლარი და ' + partWord + ' თეთრი'
        }
        return numberWord;
    }
